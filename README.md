---
name: WordPress Hack Remediation
overview: "Step-by-step plan to remediate a hacked WordPress site: remove attacker access, validate the installation for compromise indicators, and temporarily block the site from public visitors."
todos:
  - id: block-access
    content: "Phase 1: Block public access to the site (`.htaccess` 503 rule, Cloudflare, or hosting panel)"
    status: completed
  - id: change-creds
    content: "Phase 2.1: Change all passwords (WP admin, DB, FTP/SSH, hosting panel) and regenerate WP salts"
    status: completed
  - id: remove-users
    content: "Phase 2.2: Audit and remove unknown admin accounts from the database"
    status: completed
  - id: remove-plugin
    content: "Phase 2.3: Delete the suspicious plugin and check `mu-plugins/` for hidden malware"
    status: completed
  - id: find-backdoors
    content: "Phase 2.4: Scan for backdoor files (PHP in uploads, obfuscated code, recently modified files)"
    status: completed
  - id: remove-crons
    content: "Phase 2.5: List and remove any rogue WP cron jobs"
    status: completed
  - id: verify-core
    content: "Phase 3.1-3.2: Verify WordPress core and plugin checksums, replace modified files"
    status: completed
  - id: check-db
    content: "Phase 3.3: Search database for injected scripts, iframes, and suspicious options"
    status: completed
  - id: audit-config
    content: "Phase 3.4: Audit `.htaccess` and `wp-config.php` for injected code or redirects"
    status: completed
  - id: reinstall-clean
    content: "Phase 3.5: Reinstall clean copies of core, plugins, and theme"
    status: completed
  - id: harden
    content: "Phase 3.6: Update everything, set file permissions, install security plugin, enable 2FA, disable file editing"
    status: completed
  - id: go-live
    content: "Phase 4: Remove maintenance block, test site, request Google malware review, monitor logs"
    status: completed
isProject: false
---
